(function(){
  ...
  async function init(){
    try{
      ...
      document.querySelectorAll('.group').forEach(group=>{const isEducation=group.hasAttribute('data-education-dashboard-link'); const visible=isEducation||[...group.querySelectorAll('[data-page]')].some(x=>x.dataset.roleHidden!=='1'&&getComputedStyle(x).display!=='none');if(!visible)group.style.display='none';});
