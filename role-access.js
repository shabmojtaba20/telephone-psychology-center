(function(){
  ...
  async function init(){
    try{
      ...
      const {data:roles,error}=await db.rpc('admin_get_user_roles',{p_user_id:session.user.id});
      ...
      document.querySelectorAll('[data-page]').forEach(el=>{
        ...
        if(!allowed.has(p)) el.style.display='none';
      });
      document.querySelectorAll('.group').forEach(g=>{
        const visible=[...g.querySelectorAll('[data-page]')].some(x=>x.style.display!=='none');
        if(!visible) g.style.display='none';
      });
      window.__adminRoles=names;
      window.__adminAllowedPages=allowed;
      const originalShow=window.showPage;
      if(typeof originalShow==='function'){
        window.showPage=function(page){
          if(!allowed.has(page)){window.note?.('این بخش برای نقش شما مجاز نیست.',true);return false}
