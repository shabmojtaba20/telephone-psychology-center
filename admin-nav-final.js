...
function init(){
  ...
  if(!document.getElementById('admin-nav-v5-style')){
    const s=document.createElement('style');
    s.id='admin-nav-v5-style';
    s.textContent=[
      ...
      '.nav .group>.group-title{touch-action:manipulation;-webkit-tap-highlight-color:transparent;user-select:none;cursor:pointer}',
      '.nav .sub button[data-page]{pointer-events:auto!important;touch-action:manipulation;position:relative;z-index:2}'
    ].join('');
    document.head.appendChild(s);
  }

  // Bind the actual elements directly. This avoids conflicts with older
  // document-level handlers and works reliably on Android touch/click.
  nav.querySelectorAll('.group>.group-title').forEach(title=>{
    if(title.dataset.adminNavV5Bound==='1')return;
    title.dataset.adminNavV5Bound='1';
    title.type='button';
  ...
  nav.querySelectorAll('[data-page]').forEach(button=>{
    if(button.dataset.adminNavV5Bound==='1')return;
    button.dataset.adminNavV5Bound='1';
    button.type='button';
  ...
  const menu=$('menuBtn');
  if(menu && menu.dataset.adminNavV5Bound!=='1'){
    menu.dataset.adminNavV5Bound='1';
    menu.type='button';
