...
function init(){
 ...
 groups.forEach(btn=>{btn.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const group=btn.closest('.group');if(group)group.classList.toggle('open')},true)});
 ...
 const menu=$('menuBtn');if(menu)menu.addEventListener('click',event=>{event.preventDefault();$('side')?.classList.toggle('open')},true);
}
