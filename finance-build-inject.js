...
if(fs.existsSync(reportFile)){let h=fs.readFileSync(reportFile,'utf8');h=h.replace(/\s*<script[^>]+src=["']\/advanced-finance-report\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'');h=h.replace(/\s*<script[^>]+src=["']\/finance-report-reconciliation\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'');const m='<!-- advanced-finance-report-injection -->';const s=`\n${m}\n<script src="/advanced-finance-report.js?v=${version}"></script>\n<script src="/finance-report-reconciliation.js?v=${version}"></script>`;if(!h.includes(m))h=h.replace('</body>',`${s}\n</body>`);else h=h.replace(m,`${s}`);fs.writeFileSync(reportFile,h,'utf8');}
...
if(fs.existsSync(settlementFile)){let h=fs.readFileSync(settlementFile,'utf8');h=h.replace(/\s*<script[^>]+src=["']\/finance-settlement-chain\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'');const m='<!-- finance-settlement-chain-injection -->';const s=`\n${m}\n<script src="/finance-settlement-chain.js?v=${version}"></script>`;if(!h.includes(m))h=h.replace('</body>',`${s}\n</body>`);else h=h.replace(m,`${s}`);fs.writeFileSync(settlementFile,h,'utf8');}
