...
const assets=['finance-tools','finance-dashboard','finance-manager-report','commission-rules','finance-receipt-preview','finance-audit-ledger','management-kpi-dashboard','management-kpi-charts'];
for(const a of assets) adminHtml=adminHtml.replace(new RegExp('\\s*<script[^>]+src=["\\\']\\/'+a+'\\.js(?:\\?[^"\\\']*)?["\\\'][^>]*><\\/script>','gi'),'');
const adminMarker='<!-- finance-build-injection-v6 -->';
...
if(fs.existsSync(reportFile)){let h=fs.readFileSync(reportFile,'utf8');h=h.replace(/\s*<script[^>]+src=["']\/advanced-finance-report\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'');const m='<!-- advanced-finance-report-injection -->';const s=`\n${m}\n<script src="/advanced-finance-report.js?v=${version}"></script>`;if(!h.includes(m))h=h.replace('</body>',`${s}\n</body>`);else h=h.replace(m,`${m}\n<script src="/advanced-finance-report.js?v=${version}"></script>`);fs.writeFileSync(reportFile,h,'utf8');}
