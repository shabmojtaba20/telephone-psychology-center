...
if(fs.existsSync(reportFile)){
  let h=fs.readFileSync(reportFile,'utf8');
  const reportAssets=['advanced-finance-report','finance-report-reconciliation','finance-report-expense-breakdown','finance-report-export-breakdown','finance-report-excel','finance-general-ledger'];
  for(const a of reportAssets) h=h.replace(new RegExp('\\s*<script[^>]+src=["\\\']\\/'+a+'\\.js(?:\\?[^"\\\']*)?["\\\'][^>]*><\\/script>','gi'),'');
  const m='<!-- advanced-finance-report-injection -->';
  ...
  fs.writeFileSync(reportFile,h,'utf8');
}
