...
const originalLoad=window.loadReceipts;
...
async function openReview(id,path,status){const m=modal();m.querySelector('#rrMeta').innerHTML='<div><b>وضعیت:</b> '+escLocal(status||'در انتظار')+'</div>';
 ...
 const actions=m.querySelector('#rrActions');actions.innerHTML='';if(status==='pending'){actions.innerHTML=`<div class="rr-review"><input id="rrNote" placeholder="یادداشت یا علت رد"><input id="rrTrack" placeholder="کد پیگیری نهایی"><button class="btn green" id="rrApprove">تأیید رسید</button><button class="btn red" id="rrReject">رد رسید</button></div>`;$('rrApprove').onclick=async()=>{await window.reviewReceipt(id,'approved') ;m.remove()};$('rrReject').onclick=async()=>{await window.reviewReceipt(id,'rejected');m.remove()}}
}
...
function install(){
 ...
 window.__receiptReviewInstalled=true;
 window.__reviewReceipt=openReview;
 const table=$('receiptsBody')?.closest('table');if(table){const hs=table.querySelectorAll('thead th');if(hs.length>=7){hs[0].textContent='کاربر';hs[1].textContent='تاریخ';hs[2].textContent='وضعیت';hs[3].textContent='رسید';for(let i=4;i<7;i++)hs[i].remove();}}
 const old=window.loadReceipts;window.loadReceipts=loadReceiptsSimplified;loadReceiptsSimplified();
