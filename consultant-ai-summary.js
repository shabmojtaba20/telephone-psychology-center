(function(){
  ...
  async function getToken(){try{const r=await fetch(SB_URL+'/auth/v1/token?grant_type=password',{method:'GET',headers:{apikey:SB_KEY}});return r}catch(e){return null}}
  async function run(card,button){
    ...
    try{
      const sess=JSON.parse(localStorage.getItem('supabase.auth.token')||'null');
      ...
      const j=await r.json();if(!r.ok||j.error)throw new Error(j.error||'خطا در سرویس هوش مصنوعی');
