...
export async function fetchServices() { const { data, error } = await supabase.from('services').select('id,name,description,duration_minutes,price,is_active,sort_order').eq('is_active', true).order('sort_order').order('created_at'); if (error) throw error; return data ?? []; }
...
export async function startAppointment(slotId) {
  if (!slotId) throw new Error('slot_id_required');
  const { data: booking, error: bookingError } = await supabase.functions.invoke('payment-start', { body: { slot_id: slotId, service_id: window.__selectedServiceId || null } });
...
export async function sendPhoneOtp(phone) { const normalizedPhone = String(phone || '').trim(); if (!/^\+?[1-9]\d{7,14}$/.test(normalizedPhone)) throw new Error('شماره موبایل معتبر نیست.'); const { error } = await supabase.auth.signInWithOtp({ phone: normalizedPhone }); if (error) throw error; }
export async function verifyPhoneOtp(phone, token) { const normalizedPhone = String(phone || '').trim(); const normalizedToken = String(token || '').trim(); if (!/^\+?[1-9]\d{7,14}$/.test(normalizedPhone)) throw new Error('شماره موبایل معتبر نیست.'); if (!/^\d{6}$/.test(normalizedToken)) throw new Error('کد تأیید باید ۶ رقم باشد.'); const { data, error } = await supabase.auth.verifyOtp({ phone: normalizedPhone, token: normalizedToken, type: 'sms' }); if (error) throw error; return data?.session || null; }
...
// Services UI is mounted as a side effect so the existing booking/payment frontend remains intact.
