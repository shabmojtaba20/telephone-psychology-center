...
if(fs.existsSync(consultantPath)){let html=fs.readFileSync(consultantPath,'utf8');html=html.replace(/\s*<script[^>]+src=["']\/consultant-payment-status\.js(?:\?[^"']*)?["'][^>]*><\/script>/gi,'');html=injectHead(html,[panelAuth]);html=inject(html,[logoutFix,consultantAi,consultantReadonly,consultantPayment,consultantOps,theme]);fs.writeFileSync(consultantPath,html,'utf8');}
