...
if(fs.existsSync(adminPath)){let html=fs.readFileSync(adminPath,'utf8');for(const s of [roleScript,roleSwitcher,routeGuard,panelIntegration,logoutFix,adminStability,consultantManager,mediaUi,branding,theme,homepageManager,homepagePro,homepageAdmin,notifications,faqs])if(!html.includes(s))html=html.replace('</body>',`\n${s}\n</body>`);for(const s of [modernCss,homepageCss,homepageProCss])if(!html.includes(s))html=html.replace('</head>',`\n${s}\n</head>`);fs.writeFileSync(adminPath,html,'utf8');}
const consultantPath=path.join(root,'consultant-panel-professional.html');
if(fs.existsSync(consultantPath)){let html=fs.readFileSync(consultantPath,'utf8');for(const s of [consultantAi,consultantReadonly,theme])if(!html.includes(s))html=html.replace('</body>',`\n${s}\n</body>`);fs.writeFileSync(consultantPath,html,'utf8');}
...
console.log('Admin stability + Stage 9 FAQ + Stage 10 consultant manager + consultant read-only profile injected');
